...
pub async fn run_server(redis_port: u16, http_port: u16) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("Starting Redis server on port {} and HTTP server on port {}", redis_port, http_port);
    ...
    tokio::try_join!(redis_handle, http_handle).map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
